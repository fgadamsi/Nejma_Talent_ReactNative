...
class Home extends Component {
        ...
        try {
          ...
          .then((talent) => {
            if(talent!=null) {
              ...
              console.log(qwe)
              actions.setLoginDetails(qwe.talentId, qwe.talentName, qwe.talentBio, qwe.talentImage, qwe.handel, qwe.professions, qwe.talentEmail, qwe.accessToken);
  ...
  showEmailResend = ()=> {
    ...
    return <SafeAreaView style={{flex:0.07,backgroundColor: '#2D2929', justifyContent: 'center'}}>
            <View style={{minHeight: 48,flexDirection: flexDirection, marginHorizontal: 12, alignItems: 'center'}}>
              ...
              <TouchableOpacity hitSlop={{top:7, bottom:7, left:7, right:7}} disabled={isDisabled} style={{flex:0.2,height: 32,backgroundColor:  colors.themeColor, justifyContent: 'center', alignItems: 'center', borderRadius: 4}} onPress={this.emailResend}>
  ...
  reject = (reject, user, i)=> {
    ...
    Navigation.showOverlay({
      component: {
        name: 'FamCamTalent.RejectReason',
        options: {
          overlay: {
      ...
      <View style={{flex:1}}>
        <View style={{justifyContent: 'center', marginVertical: 25}}>
          <View style={{flexDirection: flexDirection, alignItems: 'center'}}>
            ...
            <View style={{flex:0.7}}>
              <Text style={{textAlign: textAlign, color:  colors.themeColor, fontSize: 20, fontFamily: lang=='en'?'SFProText-Regular':'HelveticaNeueLTArabic-Light'}}>{item.forWhome}</Text>
            </View>
          </View>
          ...
          <View style={{flexDirection: flexDirection}}>
            ...
            <TouchableOpacity hitSlop={{top:7, bottom:7, left:7, right:7}} disabled={isDisabled || refreshing} style={{flex:0.3, alignItems: lang=='en'?'flex-end':'flex-start'}} onPress={()=>this.reject(item.requestId, item.userId, index)}>
              <Text style={{textAlign: textAlign, color: colors.themeColor, fontSize: 12, fontFamily: lang=='en'?'SFProDisplay-Bold':'HelveticaNeueLTArabic-Bold'}}> {string('Home.rejectBTN')} </Text>
            </TouchableOpacity>
          </View>
